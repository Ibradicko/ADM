...
import { sampleWithRequiredData } from '../calcul-redevance.test-samples';
import { CalculRedevanceService } from '../service/calcul-redevance.service';

import { CalculRedevance } from './calcul-redevance';

vitest.useFakeTimers();

describe('CalculRedevance Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CalculRedevance;
  let fixture: ComponentFixture<CalculRedevance>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CalculRedevance);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 28461 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.calculRedevances()[0]).toEqual(expect.objectContaining({ id: 28461 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 28461 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.calculRedevances()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 28461 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to calculRedevanceService', () => {
      ...
      vitest.spyOn(service, 'getCalculRedevanceIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCalculRedevanceIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.calculRedevancesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.calculRedevancesParams()).toMatchObject(
