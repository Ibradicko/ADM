...
import codeBarresProduitResolve from './code-barres-produit-routing-resolve.service';

describe('CodeBarresProduit routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ICodeBarresProduit returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          codeBarresProduitResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(codeBarresProduitResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
