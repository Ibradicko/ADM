...
import { sampleWithRequiredData } from '../depot-stock.test-samples';
...
describe('DepotStock Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 26721 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.depotStocks()[0]).toEqual(expect.objectContaining({ id: 26721 }));
  });

  describe('trackId', () => {
    it('should forward to depotStockService', () => {
      const entity = { id: 26721 };
      vitest.spyOn(service, 'getDepotStockIdentifier');
      const id = comp.trackId(entity);
      expect(service.getDepotStockIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.depotStocksParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
