...
import { sampleWithRequiredData } from '../famille-article.test-samples';
...
describe('FamilleArticle Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: FamilleArticle;
  let fixture: ComponentFixture<FamilleArticle>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(FamilleArticle);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 29368 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.familleArticles()[0]).toEqual(expect.objectContaining({ id: 29368 }));
  });

  describe('trackId', () => {
    it('should forward to familleArticleService', () => {
      const entity = { id: 29368 };
      vitest.spyOn(service, 'getFamilleArticleIdentifier');
      const id = comp.trackId(entity);
      expect(service.getFamilleArticleIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.familleArticlesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
