...
import { sampleWithRequiredData } from '../inventaire-stock.test-samples';
import { InventaireStockService } from '../service/inventaire-stock.service';

import { InventaireStock } from './inventaire-stock';

vitest.useFakeTimers();

describe('InventaireStock Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: InventaireStock;
  let fixture: ComponentFixture<InventaireStock>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(InventaireStock);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 31192 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.inventaireStocks()[0]).toEqual(expect.objectContaining({ id: 31192 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 31192 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.inventaireStocks()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 31192 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to inventaireStockService', () => {
      ...
      vitest.spyOn(service, 'getInventaireStockIdentifier');
      const id = comp.trackId(entity);
      expect(service.getInventaireStockIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.inventaireStocksParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.inventaireStocksParams()).toMatchObject(
