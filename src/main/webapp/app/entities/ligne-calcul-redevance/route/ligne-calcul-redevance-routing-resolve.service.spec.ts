...
import ligneCalculRedevanceResolve from './ligne-calcul-redevance-routing-resolve.service';

describe('LigneCalculRedevance routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ILigneCalculRedevance returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          ligneCalculRedevanceResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(ligneCalculRedevanceResolve(mockActivatedRouteSnapshot))).rejects.toThrowError(
