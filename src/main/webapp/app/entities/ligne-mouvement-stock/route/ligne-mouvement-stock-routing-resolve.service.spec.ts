...
import ligneMouvementStockResolve from './ligne-mouvement-stock-routing-resolve.service';

describe('LigneMouvementStock routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ILigneMouvementStock returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          ligneMouvementStockResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(ligneMouvementStockResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
