...
import ligneReceptionProduitResolve from './ligne-reception-produit-routing-resolve.service';

describe('LigneReceptionProduit routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ILigneReceptionProduit returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          ligneReceptionProduitResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(ligneReceptionProduitResolve(mockActivatedRouteSnapshot))).rejects.toThrowError(
