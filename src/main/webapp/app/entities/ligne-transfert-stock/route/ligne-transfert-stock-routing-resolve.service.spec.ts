...
import ligneTransfertStockResolve from './ligne-transfert-stock-routing-resolve.service';

describe('LigneTransfertStock routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ILigneTransfertStock returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          ligneTransfertStockResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(ligneTransfertStockResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
