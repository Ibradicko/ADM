...
import { sampleWithRequiredData } from '../ligne-vente.test-samples';
...
describe('LigneVente Management Component', () => {
  ...
  let comp: LigneVente;
  let fixture: ComponentFixture<LigneVente>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(LigneVente);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 103 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.ligneVentes()[0]).toEqual(expect.objectContaining({ id: 103 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 103 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.ligneVentes()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 103 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to ligneVenteService', () => {
      const entity = { id: 103 };
      vitest.spyOn(service, 'getLigneVenteIdentifier');
      const id = comp.trackId(entity);
      expect(service.getLigneVenteIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.ligneVentesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.ligneVentesParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
