...
import lotEtiquettesResolve from './lot-etiquettes-routing-resolve.service';

describe('LotEtiquettes routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ILotEtiquettes returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          lotEtiquettesResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(lotEtiquettesResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
