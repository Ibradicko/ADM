...
import modePaiementRefResolve from './mode-paiement-ref-routing-resolve.service';

describe('ModePaiementRef routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IModePaiementRef returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          modePaiementRefResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(modePaiementRefResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
