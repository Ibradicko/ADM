...
import paiementRedevanceResolve from './paiement-redevance-routing-resolve.service';

describe('PaiementRedevance routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPaiementRedevance returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          paiementRedevanceResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(paiementRedevanceResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
