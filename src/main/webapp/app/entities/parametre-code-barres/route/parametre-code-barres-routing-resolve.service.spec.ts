...
import parametreCodeBarresResolve from './parametre-code-barres-routing-resolve.service';

describe('ParametreCodeBarres routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IParametreCodeBarres returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          parametreCodeBarresResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(parametreCodeBarresResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
