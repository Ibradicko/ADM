...
import { sampleWithRequiredData } from '../rapport-export.test-samples';
import { RapportExportService } from '../service/rapport-export.service';

import { RapportExport } from './rapport-export';

vitest.useFakeTimers();

describe('RapportExport Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: RapportExport;
  let fixture: ComponentFixture<RapportExport>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(RapportExport);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 650 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.rapportExports()[0]).toEqual(expect.objectContaining({ id: 650 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 650 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.rapportExports()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 650 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to rapportExportService', () => {
      const entity = { id: 650 };
      vitest.spyOn(service, 'getRapportExportIdentifier');
      const id = comp.trackId(entity);
      expect(service.getRapportExportIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.rapportExportsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.rapportExportsParams()).toMatchObject(
