...
import { sampleWithRequiredData } from '../reception-produit.test-samples';
import { ReceptionProduitService } from '../service/reception-produit.service';

import { ReceptionProduit } from './reception-produit';

vitest.useFakeTimers();

describe('ReceptionProduit Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ReceptionProduit;
  let fixture: ComponentFixture<ReceptionProduit>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ReceptionProduit);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 19661 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.receptionProduits()[0]).toEqual(expect.objectContaining({ id: 19661 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 19661 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.receptionProduits()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 19661 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to receptionProduitService', () => {
      ...
      vitest.spyOn(service, 'getReceptionProduitIdentifier');
      const id = comp.trackId(entity);
      expect(service.getReceptionProduitIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.receptionProduitsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.receptionProduitsParams()).toMatchObject(
