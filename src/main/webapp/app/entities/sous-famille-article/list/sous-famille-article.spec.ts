...
import { sampleWithRequiredData } from '../sous-famille-article.test-samples';

import { SousFamilleArticle } from './sous-famille-article';

vitest.useFakeTimers();

describe('SousFamilleArticle Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: SousFamilleArticle;
  let fixture: ComponentFixture<SousFamilleArticle>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(SousFamilleArticle);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 30207 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.sousFamilleArticles()[0]).toEqual(expect.objectContaining({ id: 30207 }));
  });

  describe('trackId', () => {
    it('should forward to sousFamilleArticleService', () => {
      const entity = { id: 30207 };
      vitest.spyOn(service, 'getSousFamilleArticleIdentifier');
      const id = comp.trackId(entity);
      expect(service.getSousFamilleArticleIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.sousFamilleArticlesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
