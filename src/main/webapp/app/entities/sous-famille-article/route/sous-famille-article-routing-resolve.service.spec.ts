...
import sousFamilleArticleResolve from './sous-famille-article-routing-resolve.service';

describe('SousFamilleArticle routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ISousFamilleArticle returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          sousFamilleArticleResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(sousFamilleArticleResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
