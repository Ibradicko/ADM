...
import { sampleWithRequiredData } from '../tarif-produit.test-samples';

import { TarifProduit } from './tarif-produit';

vitest.useFakeTimers();

describe('TarifProduit Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: TarifProduit;
  let fixture: ComponentFixture<TarifProduit>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TarifProduit);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 16387 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.tarifProduits()[0]).toEqual(expect.objectContaining({ id: 16387 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 16387 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.tarifProduits()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 16387 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to tarifProduitService', () => {
      const entity = { id: 16387 };
      vitest.spyOn(service, 'getTarifProduitIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTarifProduitIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.tarifProduitsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.tarifProduitsParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
