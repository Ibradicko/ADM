...
import { sampleWithRequiredData } from '../transfert-stock.test-samples';

import { TransfertStock } from './transfert-stock';

vitest.useFakeTimers();

describe('TransfertStock Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: TransfertStock;
  let fixture: ComponentFixture<TransfertStock>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TransfertStock);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 31780 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.transfertStocks()[0]).toEqual(expect.objectContaining({ id: 31780 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 31780 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.transfertStocks()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 31780 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to transfertStockService', () => {
      ...
      vitest.spyOn(service, 'getTransfertStockIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTransfertStockIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.transfertStocksParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.transfertStocksParams()).toMatchObject(
