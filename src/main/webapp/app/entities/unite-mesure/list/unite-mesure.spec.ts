...
import { sampleWithRequiredData } from '../unite-mesure.test-samples';
...
describe('UniteMesure Management Component', () => {
  ...
  let comp: UniteMesure;
  let fixture: ComponentFixture<UniteMesure>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(UniteMesure);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 4120 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.uniteMesures()[0]).toEqual(expect.objectContaining({ id: 4120 }));
  });

  describe('trackId', () => {
    it('should forward to uniteMesureService', () => {
      const entity = { id: 4120 };
      vitest.spyOn(service, 'getUniteMesureIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.uniteMesuresParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
